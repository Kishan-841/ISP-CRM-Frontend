 ...
 */
export const getRequiredDocumentTypesFor = ({ hasGst = true } = {}) => {
  return getRequiredDocumentTypes().filter((doc) => hasGst || doc.id !== 'GST_DETAILS');
};
 ...
 * when not applicable).
 */
export const getVisibleDocumentTypes = ({ hasGst = true } = {}) => {
  return getAllDocumentTypes().filter((doc) => hasGst || doc.id !== 'GST_DETAILS');
};
