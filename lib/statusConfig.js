...
export const SERVICE_ORDER_STATUS_CONFIG = {
  // New two-stage approval gates (delivery first for UPGRADE/DOWNGRADE,
  // then sales director for everything).
  PENDING_DELIVERY_APPROVAL:       { label: 'Pending Delivery',       color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200' },
  PENDING_SALES_DIRECTOR_APPROVAL: { label: 'Pending Sales Director', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200' },
  ...
  PENDING_ADMIN_DATE_APPROVAL: { label: 'Pending Admin (Date Change)', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200' },
  ...
  // Legacy states — kept for back-compat so historical orders still render
  // a labelled badge. New orders no longer enter these states.
  PENDING_APPROVAL:       { label: 'Legacy: Pending Approval',       color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' },
  PENDING_SAM_ACTIVATION: { label: 'Legacy: Pending SAM Activation', color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' },
};
...
export const DELIVERY_STAGE_CONFIG = {
  vendor_setup:         { label: 'Vendor Setup', color: 'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 border-violet-200' },
