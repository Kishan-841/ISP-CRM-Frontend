    ...
    try {
      ...
      set({
        counts: {
          ...
          // Feasibility Team counts
          feasibilityPending: data.pending || 0,
          // OPS Team counts
          opsPending: data.pending || 0,
          // Super Admin 2 / Admin approval counts
          sa2Pending: data.sa2Pending || data.pending || 0,
          // Docs Team counts
          docsPending: data.pending || 0,
          // Accounts Team counts
          accountsPending: data.pending || 0,
...
export const useUserStore = create((set, get) => ({
  ...
  fetchUsers: createFetchAction(set, '/users', 'users', 'users', 'Failed to fetch users'),
...
export const useCampaignStore = create((set, get) => ({
  ...
  // Fetch campaign data
  fetchCampaignData: async (campaignId, page = 1, limit = 10, status = null, filterType = null) => {
    set({ isLoading: true, error: null });
    try {
      ...
      const response = await api.get(`/campaigns/${campaignId}/data?${params}`);
      set({
  ...
  // End a call
  endCall: async (callLogId, status, notes, callLaterAt = null) => {
    try {
      const response = await api.post(`/campaigns/call/end/${callLogId}`, { status, notes, callLaterAt });
  ...
  createSelfCampaign: async (name, dataSource, data, assignToId = null) => {
    try {
      const response = await api.post('/campaigns/self-campaign', { name, dataSource, data, assignToId });
      const { campaigns } = get();
      set({ campaigns: [response.data.campaign, ...campaigns] });
      return {
        success: true,
...
export const useLeadStore = create((set, get) => ({
  ...
  feasibilityDisposition: async (leadId, { decision, notes, vendorInfo, vendorId }) => {
    try {
      const response = await api.post(`/leads/feasibility-team/${leadId}/disposition`, { decision, notes, vendorInfo, vendorId });
      ...
      set({
        ...
        feasibilityReviewCounts: {
          approved: decision === 'FEASIBLE' ? feasibilityReviewCounts.approved + 1 : feasibilityReviewCounts.approved,
          rejected: decision === 'NOT_FEASIBLE' ? feasibilityReviewCounts.rejected + 1 : feasibilityReviewCounts.rejected
        }
      });
      return { success: true, lead: response.data.lead, message: response.data.message };
    } catch (error) {
  ...
  opsTeamDisposition: async (leadId, { decision, reason }) => {
    try {
      const response = await api.post(`/leads/ops-team/${leadId}/disposition`, { decision, reason });
  ...
  nocGenerateCircuitId: async (leadId) => {
    try {
      const response = await api.post(`/leads/noc/${leadId}/generate-circuit`);
      const { nocQueue, selectedNocLead } = get();
      const lead = response.data.lead;
      const updateFields = {
...
export const useVendorStore = create((set, get) => ({
  ...
  createVendor: async (vendorData) => {
    ...
    try {
      const formData = new FormData();
      // Append text fields
      const textFields = ['vendorEntityType', 'companyName', 'individualName', 'gstNumber', 'contactPerson', 'email', 'phone', 'panNumber', 'address', 'city', 'state', 'category', 'accountNumber', 'ifscCode', 'accountName', 'bankName', 'branchName'];
