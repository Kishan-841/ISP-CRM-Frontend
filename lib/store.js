...
export const useAuthStore = create((set, get) => ({
  ...
  login: async (email, password) => {
    try {
      ...
      try { useMeetingReminderStore.getState().reset(); } catch {}
...
// ============================================================
// Meeting reminders — driven by server cron, displayed as stacked modals
// ============================================================

export const useMeetingReminderStore = create((set, get) => ({
  // queue of active reminders the user hasn't dismissed yet
  reminders: [],

  pushReminder: (payload) => {
    if (!payload?.meetingId) return;
    // Dedup in case the server ever re-fires (client side defense-in-depth)
    const existing = get().reminders.find(
      (r) => r.meetingId === payload.meetingId && r.source === payload.source,
    );
    if (existing) return;
    set((s) => ({
  ...
  dismissReminder: (meetingId, source) =>
    set((s) => ({
      reminders: s.reminders.filter(
        (r) => !(r.meetingId === meetingId && r.source === source),
      ),
    })),
