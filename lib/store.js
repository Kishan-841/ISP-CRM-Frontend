    ...
    try {
      ...
      set({
        counts: {
          ...
          // Delivery Request Approval counts (for Super Admin)
          ...
          // New state-machine gates: Delivery first, then Sales Director.
          // The legacy `samActivationPending` is gone — the SAM activation
          // step was removed from the pipeline entirely.
          deliveryOrderApprovalPending: data.deliveryOrderApprovalPending || 0,
          salesDirectorPending: data.salesDirectorPending || 0,
          ...
          // Quick-disconnect inbox (SAM-raised commercial changes pending super-admin decision)
          quickDisconnectPending: data.quickDisconnectPending || 0,
        },
        countsLoading: false,
...
export const useAuthStore = create((set, get) => ({
  ...
  logout: async () => {
    try {
      ...
      // Server-side audit is best-effort — never block client logout on it.
    }
    localStorage.removeItem('token');
    localStorage.removeItem('user');
...
export const useUserStore = create((set, get) => ({
  ...
  // Reveal a user's plaintext password (SUPER_ADMIN / MASTER only). The
  // server audit-logs every successful reveal in StatusChangeLog.
  viewUserPassword: async (id) => {
    try {
      const response = await api.get(`/users/${id}/password`);
      return { success: true, data: response.data };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to view password',
      };
    }
  },

  // Fetch user dashboard stats (admin viewing user's dashboard)
  // Admin / master / sales-director view of an ISR's dashboard. Routes
  // through /campaigns/dashboard/stats?userId= (the same endpoint the ISR
  // hits when they log in) so master sees exactly what the ISR sees —
  // historically this called /users/:id/dashboard which drifted away
  // from the ISR's own view.
  fetchUserDashboardStats: async (userId, period = 'last7days') => {
    try {
      const response = await api.get(`/campaigns/dashboard/stats?userId=${userId}&period=${period}`);
      return { success: true, data: response.data };
    } catch (error) {
      return {
        success: false,
...
export const useCampaignStore = create((set, get) => ({
  ...
  // Self Leads (BDM /dashboard/self-leads) — replaces the old N×500 fetch
  // pattern. Stats endpoint provides the six tab-card counts; queue endpoint
  // returns one paginated page of CampaignData rows for whichever tab is
  // active (pending or called) and respects the called-tab status filter
  // server-side.
  fetchSelfLeadsStats: async ({ campaignId } = {}) => {
    try {
      const params = new URLSearchParams();
      if (campaignId) params.set('campaignId', campaignId);
      const qs = params.toString() ? `?${params.toString()}` : '';
      const response = await api.get(`/campaigns/self-leads/stats${qs}`);
      return {
        success: true,
        stats: response.data?.stats || {},
        campaignIds: response.data?.campaignIds || [],
      };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to fetch self-leads stats',
        stats: {},
        campaignIds: [],
      };
    }
  },
  fetchSelfLeadsQueue: async ({ campaignId, tab = 'pending', status = null, page = 1, limit = 25, search = '' } = {}) => {
    try {
      const params = new URLSearchParams({ page, limit, tab });
      if (campaignId) params.set('campaignId', campaignId);
      if (status && status !== 'all') params.set('status', status);
      if (search && search.trim()) params.set('search', search.trim());
      const response = await api.get(`/campaigns/self-leads/queue?${params.toString()}`);
      return {
        success: true,
      ...
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to fetch self-leads queue',
        data: [],
...
export const useLeadStore = create((set, get) => ({
  ...
  // Opportunity Pipeline tab badges. The page used to derive these from a
  // 500-row in-memory bucketing; this endpoint just returns 8 counts so we
  // no longer need to fetch rows we won't render. Server-scoped to the
  // user's role-based visibility (BDM sees their own, TL/admin sees all).
  fetchOpportunityPipelineStats: async () => {
    try {
      const response = await api.get('/leads/opportunity-pipeline/stats');
      return { success: true, stats: response.data?.stats || {} };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to fetch pipeline stats',
        stats: {},
      };
    }
  },
  ...
  // Lead Buckets — admin / master / super-admin / sales-director view of
  // where every lead is sitting right now (BDM / Feasibility / OPS / etc.).
  // Returns { summary, leads, pagination } in one call.
  fetchLeadBuckets: async ({ bucket = '', page = 1, limit = 25, search = '' } = {}) => {
    try {
      const params = new URLSearchParams({ page: String(page), limit: String(limit) });
      if (bucket) params.set('bucket', bucket);
      if (search) params.set('search', search);
      const response = await api.get(`/leads/buckets?${params.toString()}`);
      return { success: true, ...response.data };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to fetch lead buckets',
      };
    }
  },
  ...
  uploadDocument: async (leadId, documentType, file, metadata = {}) => {
    try {
      ...
      // "Others" documents carry a user-supplied name; the server mints an
      // OTHER_<id> key per upload so a lead can hold several of them.
      if (metadata.label) {
  ...
  pushToDocsVerificationTyped: async (leadId, notes, testMode = false, financialDetails = {}, hasGst = true) => {
    try {
  ...
  // Sales Director — revise ARC/OTC on an SA2-approved lead. The only path
  // that can change pricing after approval. Stamps the lead with the
  // quotationRevised* fields so a "Quotation Revised" badge surfaces in the UI.
  reviseQuotation: async (leadId, { arcAmount, otcAmount, reason }) => {
    try {
      const response = await api.post(`/leads/${leadId}/revise-quotation`, {
        arcAmount, otcAmount, reason,
      });
      const updated = response.data.lead;
      const { leads, accountsQueue } = get();
      set({
        leads: leads.map(l => (l.id === leadId ? { ...l, ...updated } : l)),
        accountsQueue: accountsQueue.map(l => (l.id === leadId ? { ...l, ...updated } : l)),
      });
      return { success: true, lead: updated, message: response.data.message };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to revise quotation',
      };
    }
  },
  ...
  // ========== LEGACY CUSTOMER ONBOARDING (Accounts ↔ Delivery) ==========
  // Standalone showcase flow. Accounts add (no billing) → Delivery sets delivery
  // date → Accounts set billing → COMPLETED. Isolated from the Lead pipeline.
  legacyLoading: false,
  legacyImportResult: null,

  // Accounts: create one (no billing fields)
  legacyCreateSingle: async (customerData) => {
    set({ legacyLoading: true });
    try {
      const response = await api.post('/legacy-customers/single', customerData);
      set({ legacyLoading: false });
      return { success: true, data: response.data };
    } catch (error) {
      set({ legacyLoading: false });
  ...
  // Accounts: bulk import from Excel
  legacyCreateBulk: async (rows) => {
    set({ legacyLoading: true, legacyImportResult: null });
    try {
      const response = await api.post('/legacy-customers/bulk', { rows });
      set({ legacyImportResult: response.data, legacyLoading: false });
      return { success: true, data: response.data };
    } catch (error) {
      set({ legacyLoading: false });
      return { success: false, error: error.response?.data?.message || 'Import failed' };
  ...
  // Delivery: queue of customers awaiting delivery date
  legacyGetDeliveryQueue: async (params = {}) => {
    try {
      const response = await api.get('/legacy-customers/delivery-queue', { params });
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, error: error.response?.data?.message || 'Failed to load queue' };
    }
  },

  // Delivery: record delivery date
  legacySetDeliveryDate: async (id, deliveryDate) => {
    set({ legacyLoading: true });
    try {
      const response = await api.post(`/legacy-customers/${id}/delivery`, { deliveryDate });
      set({ legacyLoading: false });
      return { success: true, data: response.data };
    } catch (error) {
      set({ legacyLoading: false });
      return { success: false, error: error.response?.data?.message || 'Failed to save delivery date' };
    }
  },

  // Accounts: queue of customers returned from delivery, awaiting billing
  legacyGetBillingQueue: async (params = {}) => {
    try {
      const response = await api.get('/legacy-customers/billing-queue', { params });
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, error: error.response?.data?.message || 'Failed to load queue' };
    }
  ...
  // Accounts: record billing date + cycle (completes the customer)
  legacySetBilling: async (id, billDate, billingCycle) => {
    set({ legacyLoading: true });
    try {
      const response = await api.post(`/legacy-customers/${id}/billing`, { billDate, billingCycle });
      set({ legacyLoading: false });
      return { success: true, data: response.data };
    } catch (error) {
      set({ legacyLoading: false });
      return { success: false, error: error.response?.data?.message || 'Failed to save billing' };
    }
  },

  // Accounts: record FTB (first time bill) amount + date on a completed customer
  legacySetFtb: async (id, ftbAmount, ftbReceivedDate) => {
    set({ legacyLoading: true });
    try {
      const response = await api.post(`/legacy-customers/${id}/ftb`, { ftbAmount, ftbReceivedDate });
      set({ legacyLoading: false });
      return { success: true, data: response.data };
    } catch (error) {
      set({ legacyLoading: false });
      return { success: false, error: error.response?.data?.message || 'Failed to save FTB' };
    }
  },

  // Accounts: full list for the Total Customers table page
  legacyListCustomers: async (params = {}) => {
    try {
      const response = await api.get('/legacy-customers', { params });
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, error: error.response?.data?.message || 'Failed to load customers' };
    }
  },

  // Accounts: dashboard stat counts
  legacyGetStats: async () => {
    try {
      const response = await api.get('/legacy-customers/stats');
      return { success: true, data: response.data?.data };
    } catch (error) {
  ...
  // Edit an existing circuit ID (NOC Head) — no stage change.
  nocUpdateCircuitId: async (leadId, circuitId) => {
    try {
      const response = await api.patch(`/leads/noc/${leadId}/customer-circuit`, { circuitId });
      const { nocQueue, selectedNocLead } = get();
      set({
        nocQueue: nocQueue.map(l => (l.id === leadId ? { ...l, circuitId } : l)),
        selectedNocLead: selectedNocLead?.id === leadId ? { ...selectedNocLead, circuitId } : selectedNocLead
      });
      return { success: true, lead: response.data.lead, message: response.data.message };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to update circuit ID'
      };
    }
  },

  // Edit an existing customer username (NOC Head) — backend syncs customerUserId.
  nocUpdateCustomerUsername: async (leadId, username) => {
    try {
      const response = await api.patch(`/leads/noc/${leadId}/customer-username`, { username });
      const { nocQueue, selectedNocLead } = get();
      const updateFields = { customerUsername: username, customerUserId: username };
      set({
        nocQueue: nocQueue.map(l => (l.id === leadId ? { ...l, ...updateFields } : l)),
        selectedNocLead: selectedNocLead?.id === leadId ? { ...selectedNocLead, ...updateFields } : selectedNocLead
      });
      return { success: true, lead: response.data.lead, message: response.data.message };
    } catch (error) {
      return {
  ...
  // Create a supplementary ("Add More Material") request against an existing one.
  // The lead stays in its current stage (e.g. NOC) — backend flags it isSupplementary.
  createSupplementaryRequest: async (parentId, requestData) => {
    try {
      const response = await api.post(`/delivery-requests/${parentId}/add-material`, requestData);
      return { success: true, request: response.data.request, message: response.data.message };
    } catch (error) {
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to add material'
      };
    }
  },
  ...
  // Read-only list for the store's "Pending Approval" tab — requests still
  // awaiting Super Admin approval. Kept in its own slice so it never clobbers
  // the Approved/Assigned tab data in `deliveryRequests`.
  storePendingRequests: [],
  storePendingCount: 0,
  fetchPendingApprovalForStore: async () => {
    set({ isLoading: true, error: null });
    try {
      const response = await api.get('/delivery-requests/store/pending-approval');
      set({
        storePendingRequests: response.data.requests,
        storePendingCount: response.data.stats?.pendingApproval || 0,
        isLoading: false
      });
      return { success: true, requests: response.data.requests };
    } catch (error) {
...
export const useVendorStore = create((set, get) => ({
  ...
  // Upload vendor documents — also used by the creator (e.g. DELIVERY_TEAM)
  // to resubmit a REJECTED vendor with corrected info + re-uploaded docs.
    ...
    try {
      ...
      textFields.forEach(field => {
        if (docData[field] !== undefined && docData[field] !== null && docData[field] !== '') {
          formData.append(field, docData[field]);
        }
      });
...
// Audit Log Store
// v1 single-select: actorIds/entityTypes/actions take only [0]. Array shape
// future-proofs the state for backend multi-select without re-shaping.
export const useAuditStore = create((set, get) => ({
  ...
  filters: {
    ...
    actorIds: [], entityTypes: [], actions: [],
    actorType: '',
    ipAddress: '', eventType: '', status: '',
    entityId: '',
  },

  setFilter(key, value) {
    set(state => ({ filters: { ...state.filters, [key]: value }, nextCursor: null, items: [] }));
    get().fetchEvents();
  },
  ...
  async fetchEvents(append = false) {
    ...
    try {
      ...
      if (filters.actorIds[0])    params.set('actorId', filters.actorIds[0]);
      if (filters.entityTypes[0]) params.set('entityType', filters.entityTypes[0]);
      if (filters.actions[0])     params.set('action', filters.actions[0]);
      ...
      set(state => ({
        items: append ? [...state.items, ...res.data.items] : res.data.items,
...
// ============================================================
// Commercial Change / Quick Disconnect inbox store
// ============================================================
export const useCommercialChangeStore = create((set, get) => ({
  items: [],
  ...
  fetchList: async ({ status = 'PENDING', page = 1, limit = 20, search = '' } = {}) => {
    set({ isLoading: true, error: null });
    try {
      const params = new URLSearchParams({ status, page: String(page), limit: String(limit) });
      if (search) params.set('search', search);
      const res = await api.get(`/commercial-changes/queue?${params.toString()}`);
      set({
  ...
  fetchById: async (id) => {
    try {
      const res = await api.get(`/commercial-changes/${id}`);
  ...
  fetchPendingCount: async () => {
    try {
      const res = await api.get('/commercial-changes/sidebar-counts');
      set({ pendingCount: res.data.pending || 0 });
      return { success: true, pending: res.data.pending || 0 };
    } catch (e) {
      return { success: false, error: e.response?.data?.message };
    }
  },
  ...
  decide: async (id, decision, note) => {
    try {
      const res = await api.patch(`/commercial-changes/${id}/decide`, { decision, note });
      return { success: true, message: res.data.message, item: res.data.item, serviceOrder: res.data.serviceOrder };
    } catch (e) {
      return { success: false, error: e.response?.data?.message || e.message };
    }
  },
  ...
  cancel: async (id, note) => {
    try {
      const res = await api.patch(`/commercial-changes/${id}/cancel`, { note });
      return { success: true, message: res.data.message, item: res.data.item };
    } catch (e) {
      return { success: false, error: e.response?.data?.message || e.message };
    }
  },
...
// ============================================================
// Team Performance — BDM Team Leader oversight store
// ============================================================
export const useTeamPerformanceStore = create((set) => ({
  // Summary state
  members: [],            // [{ id, name, email, role }]
  byMember: [],           // [{ id, name, totalLeads, qualified, inPipeline, converted, dropped, totalArc, conversionRate }]
  totals: null,           // { totalLeads, qualified, ... }
  summaryLoading: false,
  summaryError: null,
  ...
  fetchSummary: async ({ period, fromDate, toDate } = {}) => {
    ...
    try {
      ...
      const res = await api.get(`/leads/team-performance/summary${qs ? `?${qs}` : ''}`);
      set({
        members: res.data.members || [],
        byMember: res.data.byMember || [],
        totals: res.data.total || null,
        summaryLoading: false,
      });
      return { success: true };
    } catch (e) {
      set({ summaryLoading: false, summaryError: e.response?.data?.message || e.message });
      return { success: false, error: e.response?.data?.message };
    }
  },

  fetchLeads: async ({ bdmId = 'all', status, search = '', page = 1, limit = 25, bucket, period, fromDate, toDate, source } = {}) => {
    set({ leadsLoading: true, leadsError: null });
    try {
      const params = new URLSearchParams({ bdmId, page: String(page), limit: String(limit) });
      if (status && status !== 'all') params.set('status', status);
      if (search) params.set('search', search);
      ...
      const res = await api.get(`/leads/team-performance/leads?${params.toString()}`);
      set({
      ...
      set({ leadsLoading: false, leadsError: e.response?.data?.message || e.message });
      return { success: false, error: e.response?.data?.message };
    }
  },

  // Pull EVERY row that matches the current filters (not just the visible
  // page) for Excel export. Pages through at the backend's max limit (500)
  // so a large team still exports completely. Does NOT touch `leads` /
  // `pagination` — the on-screen table is untouched.
  exportLeads: async ({ bdmId = 'all', status, search = '', bucket, period, fromDate, toDate, source } = {}) => {
    const PAGE_SIZE = 500;
    const all = [];
    let page = 1;
    let totalPages = 1;
    try {
      do {
        const params = new URLSearchParams({ bdmId, page: String(page), limit: String(PAGE_SIZE) });
        if (status && status !== 'all') params.set('status', status);
        if (search) params.set('search', search);
        if (bucket && bucket !== 'all') params.set('bucket', bucket);
        if (source && source !== 'all') params.set('source', source);
