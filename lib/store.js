...
export const useTeamPerformanceStore = create((set) => ({
  ...
  fetchSummary: async () => {
    ...
    try {
      const res = await api.get('/leads/team-performance/summary');
      set({
  ...
  fetchLeads: async ({ bdmId = 'all', status, search = '', page = 1, limit = 25 } = {}) => {
  ...
  exportLeads: async ({ bdmId = 'all', status, search = '' } = {}) => {
    const PAGE_SIZE = 500;
    const all = [];
    let page = 1;
    let totalPages = 1;
