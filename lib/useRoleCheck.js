...
export function useRoleCheck() {
  ...
  return {
    ...
    isSalesDirector: user?.role === 'SALES_DIRECTOR' || isMaster,

    // Combined permission checks
    canApprovePO: user?.role === 'SUPER_ADMIN' || user?.role === 'ADMIN' || isMaster,
    canApproveDeliveryRequest: user?.role === 'SUPER_ADMIN' || isMaster,
    // Only Sales Director (and master) can revise pricing after SA2 approval.
    canRevisePricing: user?.role === 'SALES_DIRECTOR' || isMaster,
...
/**
 * Mirrors backend src/utils/roleHelper.js canViewAuditLog.
 * Widen later by editing this single function.
 */
