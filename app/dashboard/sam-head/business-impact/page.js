...
export default function SAMHeadBusinessImpact() {
  ...
  useEffect(() => {
    if (user && user.role !== 'SAM_HEAD' && user.role !== 'SUPER_ADMIN' && user.role !== 'SALES_DIRECTOR' && user.role !== 'MASTER') {
  ...
  useEffect(() => {
    if (user?.role === 'SAM_HEAD' || user?.role === 'SUPER_ADMIN' || user?.role === 'SALES_DIRECTOR' || user?.role === 'MASTER') {
      fetchData();
    }
  }, [user, fetchData]);

  useSocketRefresh(() => { fetchData(); }, { enabled: user?.role === 'SAM_HEAD' || user?.role === 'SUPER_ADMIN' || user?.role === 'SALES_DIRECTOR' || user?.role === 'MASTER' });
  ...
  if (!user || (user.role !== 'SAM_HEAD' && user.role !== 'SUPER_ADMIN' && user.role !== 'SALES_DIRECTOR' && user.role !== 'MASTER')) return null;
