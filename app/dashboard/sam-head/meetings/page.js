...
export default function SAMHeadMeetings() {
  ...
  }, [selectedExecutive, startDate, endDate]);

  const fetchCustomers = useCallback(async () => {
    try {
      const response = await api.get('/sam/customers/invoiced?assigned=true&limit=200');
  ...
  }, [user, fetchExecutives, fetchMeetings, fetchCustomers]);
  ...
  return (
    <>
      ...
      <PageHeader title="All Meeting MOMs" description="View and manage meeting minutes across all SAM executives">
        <Button
          onClick={openAddMOM}
      ...
      <DataTable
        ...
        columns={[
          {
            ...
            render: (row) => (
              <div>
                <p className="font-medium text-slate-900 dark:text-slate-100">{formatDate(row.meetingDate)}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{formatTime(row.meetingDate)}</p>
              </div>
            )
          },
          {
          ...
          },
          {
            ...
            label: 'Executive',
            render: (row) => (
              <span className="text-slate-900 dark:text-slate-100">{row.samExecutive?.name || '-'}</span>
            )
          },
          {
            key: 'type',
        ...
        defaultPageSize={10}
        searchable={true}
        searchPlaceholder="Search by customer or executive..."
        searchKeys={['customer.campaignData.company', 'customer.campaignData.name', 'samExecutive.name']}
        onRowClick={(row) => router.push(`/dashboard/sam-head/meetings/${row.id}`)}
        emptyMessage="No MOMs found"
        emptySubtitle="No meeting minutes match your filter criteria."
        filters={
        ...
        actions={(row) => (
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(`/dashboard/sam-head/meetings/${row.id}`)}
            >
              View
            </Button>
            {row.momEmailSentAt ? (
              ...
              <Button
                size="sm"
                onClick={() => router.push(`/dashboard/sam-head/meetings/${row.id}?sendEmail=true`)}
                className="bg-orange-600 hover:bg-orange-700 text-white"
              >
                <svg className="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      ...
                      {momForm.clientParticipants.map((p, i) => (
                        <div key={i} className="flex gap-2 items-center">
                          ...
                          {momForm.clientParticipants.length > 1 && (
                            <button type="button" onClick={() => setMomForm(f => ({ ...f, clientParticipants: f.clientParticipants.filter((_, idx) => idx !== i) }))} className="text-slate-400 hover:text-red-500 shrink-0 text-lg leading-none">&times;</button>
                          )}
                        </div>
                      ))}
                    </div>
                      ...
                      {momForm.gazonParticipants.map((p, i) => (
                        <div key={i} className="flex gap-2 items-center">
                          ...
                          {momForm.gazonParticipants.length > 1 && (
                            <button type="button" onClick={() => setMomForm(f => ({ ...f, gazonParticipants: f.gazonParticipants.filter((_, idx) => idx !== i) }))} className="text-slate-400 hover:text-red-500 shrink-0 text-lg leading-none">&times;</button>
                          )}
                        </div>
                      ))}
                    </div>
